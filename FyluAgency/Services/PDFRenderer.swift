...
enum PDFRenderer {
    ...
    @MainActor
    static func renderQuote(_ quote: Quote, workspace: Workspace) -> Data? {
        guard let customer = quote.customer else { return nil }
        let view = QuoteTemplate(quote: quote, customer: customer, workspace: workspace)
        return renderTemplate(view)
    }

    @MainActor
    private static func renderTemplate<V: View>(_ view: V) -> Data? {
        // A4 at 72 dpi = 595 x 842 pt
        let pageSize = CGSize(width: 595, height: 842)
        let framed = view.frame(width: pageSize.width, height: pageSize.height)
...
struct QuoteTemplate: View {
        ...
        HStack(alignment: .top) {
            ...
            VStack(alignment: .trailing, spacing: 4) {
                Text("ANGEBOT")
                    ...
                    .tracking(1.5)
                    .foregroundColor(primary)
                metaRow("Nr.", quote.number)
                metaRow("Datum", DateFmt.short(quote.date))
    ...
    private var itemsTable: some View {
        VStack(spacing: 0) {
            ...
            ForEach(quote.items.sorted(by: { $0.order < $1.order }), id: \.id) { item in
                HStack {
                    Text(item.details).frame(maxWidth: .infinity, alignment: .leading)
                    Text(item.quantity.removingDecimalsIfWhole())
                        .frame(width: 50, alignment: .trailing)
    ...
    private var totalsBlock: some View {
        VStack(spacing: 4) {
            totalRow("Netto", Money.format(quote.subtotal))
            totalRow("MwSt. \(String(format: "%.0f", quote.vatRate))%", Money.format(quote.vatAmount))
            Divider().padding(.vertical, 4)
            HStack {
                Text("Gesamt").font(.system(size: 12, weight: .bold))
                Spacer()
                Text(Money.format(quote.total)).font(.system(size: 12, weight: .bold))
            }
            .foregroundColor(primary)
        }
    }
    ...
    private var validityBlock: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text("Hinweis").font(.system(size: 9, weight: .semibold))
            if let validUntil = quote.validUntil {
                Text("Dieses Angebot ist gültig bis \(DateFmt.short(validUntil)). Mit Annahme kommt der Vertrag zwischen Ihnen und \(workspace.businessName) zustande.")
                    .font(.system(size: 9))
                    .foregroundColor(.black.opacity(0.7))
            } else {
                Text("Mit Annahme dieses Angebots kommt der Vertrag zwischen Ihnen und \(workspace.businessName) zustande.")
                    .font(.system(size: 9))
                    .foregroundColor(.black.opacity(0.7))
            }
        }
    ...
    private var footer: some View {
        HStack {
            Text(workspace.quoteFooter ?? "Wir freuen uns auf Ihre Rückmeldung.")
            Spacer()
            Text(workspace.businessName)
        }
        .font(.system(size: 8))
